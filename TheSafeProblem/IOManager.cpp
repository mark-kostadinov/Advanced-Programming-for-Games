...
void IOManager::ParseSolutionCount(std::list<std::string>& lines, std::vector<int>& digitsVector)
{
	...
	{
		PrintToConsole("\nException caught when trying to parse the solution count. Explanatory string: " + ToString(e.what()), 1);
		exit(1);
	}
...
void IOManager::UnlockSafeFileUI()
{
	...
	if (ans == 'y')
	{
		...
		PrintToConsole("\nIt took " + std::to_string(perfTimer->GetTimeSec()) + "s to perform that action.", 1);
...
{
	ClearSTLVector(roots);
	ClearSTLVector(uHashes);
	ClearSTLVector(lHashes);
	ClearSTLVector(pHashes);
	ClearSTLVector(cns);
	ClearSTLVector(lns);
	ClearSTLVector(hns);
	ClearSTLVector(validityList);
}
...
void IOManager::UnlockUsingRNG(const bool hasBonusMultiSafe)
{
	...
	for (int i = 0; i < solutionCount; i++)
	{
		...
		while (!finished)
		{
			lockedUHash.SetDigits(Number::GenerateRandomFourDigits());
			lockedLHash.SetDigits(Number::GenerateRandomFourDigits());
			lockedPHash.SetDigits(Number::GenerateRandomFourDigits());

			safe.LockTheSafe(roots.at(i), lockedUHash, lockedLHash, lockedPHash);
			safe.UnlockTheSafe(lns, hasBonusMultiSafe);

			// When it unlocks, capture the hashes to output afterwards
			if (!safe.IsLocked())
			{
				uHashes.push_back(lockedUHash);
				lHashes.push_back(lockedLHash);
				pHashes.push_back(lockedPHash);
				...
				for (int k = 0; k < numberOfLocksPerSafe; k++)
				{
					...
					hns.push_back(safe.GetCombinationLocksVector().at(k)->GetHN());
				}
				finished = true;
			}
			else
